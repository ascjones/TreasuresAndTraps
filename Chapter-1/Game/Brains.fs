...
*)
    ...
    let learn (brain:Brain) (exp:Experience) =
        ...
        let existingStrategyVal = 
            match brain |> Map.tryFind strategy with
            | Some s -> s
            | None -> 0.0
        let newStrategyVal = (1.0 - alpha) * existingStrategyVal + exp.Reward
        brain |> Map.add strategy newStrategyVal
...
*)
    let decide (brain:Brain) (state:State) =
        let strats = 
            brain 
            |> Map.filter (fun s _ -> s.State = state)
            |> Seq.sortBy (fun kvp -> kvp.Value)

        if strats |> Seq.empty then
            randomDecide ()
        else 
            let bestStrat = strats |> Seq.last
            bestStrat.Key.Action